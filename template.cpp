...
{
	return vec3( v.x * mat.cell[0] + v.y * mat.cell[1] + v.z * mat.cell[2] + mat.cell[3],
				 v.x * mat.cell[4] + v.y * mat.cell[5] + v.z * mat.cell[6] + mat.cell[7],
				 v.x * mat.cell[8] + v.y * mat.cell[9] + v.z * mat.cell[10] + mat.cell[8] );
}

vec3 operator * ( const vec3 &v, const mat4 &mat ) 
{ 
	return vec3( v.x * mat.cell[0] + v.y * mat.cell[1] + v.z * mat.cell[2] + mat.cell[3],
				 v.x * mat.cell[4] + v.y * mat.cell[5] + v.z * mat.cell[6] + mat.cell[7],
				 v.x * mat.cell[8] + v.y * mat.cell[9] + v.z * mat.cell[10] + mat.cell[8] );
	...
	while ( !exitapp )
	{
		...
		while ( SDL_PollEvent( &event ) )
		{
			...
			case SDL_KEYDOWN:
				...
				InputHandler::KeyDown( event.key.keysym.scancode );
				break;
			case SDL_KEYUP:
				InputHandler::KeyUp( event.key.keysym.scancode );
				break;
			case SDL_MOUSEMOTION:
				InputHandler::MouseMove( event.motion.x, event.motion.y );
				break;
			case SDL_MOUSEBUTTONUP:
				InputHandler::MouseUp( event.button.button );
				break;
			case SDL_MOUSEBUTTONDOWN:
				InputHandler::MouseDown( event.button.button );
				break;
			case SDL_MOUSEWHEEL:
				InputHandler::MouseWheelMove(event.wheel.x, event.wheel.y);
			default:
				break;
			}
		}
