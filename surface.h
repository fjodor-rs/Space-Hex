...
{
	const unsigned int a1 = ( a_Color1 & ALPHAMASK ) >> 24;
	if ( a1 == 0 ) return a_Color2;
	const unsigned int r = ( ( a_Color1 & REDMASK ) >> 16 ) * a1 + ( ( a_Color2 & REDMASK ) >> 16 ) * ( 255 - a1 );
	const unsigned int g = ( ( a_Color1 & GREENMASK ) >> 8 ) * a1 + ( ( a_Color2 & GREENMASK ) >> 8 ) * ( 255 - a1 );
	const unsigned int b = ( a_Color1 & BLUEMASK ) * a1 + ( a_Color2 & BLUEMASK ) * ( 255 - a1 );
	const unsigned r1 = r >> 8;
	const unsigned g1 = g >> 8;
	const unsigned b1 = b >> 8;
...
class Sprite
{
  public:
	...
	void DrawScaled( int a_X, int a_Y, int a_Width, int a_Height, Surface *a_Target, float zoom );
